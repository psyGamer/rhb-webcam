...
find ${WEBCAM_ALPGR_SNIPPET:?}/$1 -size 0 -print -delete
...
ls ${WEBCAM_ALPGR_SNIPPET:?}/$1 | while read -r file ; do
    echo "file '${WEBCAM_ALPGR_SNIPPET:?}/$1/$file'" >> ${ARCHIVE_FILE_ALPGR:?}
done
...
if [ "$HARDWARE_ACCELERATION" == "intel" ]; then
    ...
    #        -movflags +faststart -c:v hevc_nvenc -cq 42 -preset p7 -tune hq ${WEBCAM_ALPGR_DAILY:?}/$1.mp4
    #fi
