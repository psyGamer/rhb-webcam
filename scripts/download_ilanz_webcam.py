...
# url = "https://alt.spitalilanz.ch/fileadmin/webcam/rssi_webcam.jpg"
url = "https://www.rss.ch/fileadmin/webcam/rssi_webcam.jpg"
save_dir = os.getenv("WEBCAM_ILANZ_SNIPPET")
...
while True:
    ...
    try:
        ...
        image_time = datetime.strptime(last_changed, "%a, %d %b %Y %H:%M:%S %Z").replace(tzinfo=tz_gmt).astimezone(tz_local) - timedelta(seconds=1)
        image_dir  = image_time.strftime("%Y-%m-%d")
        image_name = image_time.strftime("%Y-%m-%d_%H-%M-%S.jpg")

        print(f"New image for 'Ilanz': {image_name}")
        print(res.headers)
