...
pub fn filisurLatest(ctx: *tk.Context, db: *fr.Session) !void {
    const query = db.raw("SELECT file FROM " ++ database.FilisurCapture.sql_table_name, .{})
        .orderBy("file DESC");
...
/// Show the latest capture from the Livestream webcam
pub fn livestreamLatest(ctx: *tk.Context, db: *fr.Session) !void {
    const query = db.raw("SELECT file FROM " ++ database.LivestreamCapture.sql_table_name, .{})
        .orderBy("file DESC");
